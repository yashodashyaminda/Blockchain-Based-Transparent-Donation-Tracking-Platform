...
//     // --- IPFS INTEGRATION ADDED HERE ---
//     // 4. Validate file existence from multer request
//     if (!req.file) {
//       return res.status(400).json({
//         success: false,
...
//     // 5. Upload document file to IPFS and retrieve hash CID
//     const ipfsCID = await uploadFileToIPFS(req.file);

//     // 6. Create proof document record with IPFS CID in MongoDB
//     const proof = await Proof.create({
//       campaignId,
//       ngoId: req.user._id,
//       title,
//       ipfsCID, // Saving decentralized storage hash
//       isApproved: false,
//     });
...
exports.createProof = async (req, res) => {
  ...
  try {
    ...
    if (!campaignId || !title) {
      return res.status(400).json({
        success: false,
        message: 'Please provide campaignId and proof title',
      });
    }
    ...
    // 7. Create proof document record with IPFS CID in MongoDB
    const proof = await Proof.create({
      campaignId,
      ngoId: req.user._id,
      title,
